...
export default function AdminCreatorsPage() {
  ...
  const load = useCallback(async (status: 'PENDING' | 'APPROVED' | 'REJECTED') => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/creators/admin/applications?status=${status}`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      });
      if (!res.ok) return;
      const data: Applicant[] = await res.json();
      setApplicants(data.map((a) => ({ ...a, status })));
        ...
        {(['PENDING', 'APPROVED', 'REJECTED'] as const).map((t) => (
          ...
          >
            {t === 'PENDING' && counts.PENDING > 0 && (
              <span className="flex h-4 w-4 items-center justify-center rounded-full bg-amber-500 text-[10px] font-extrabold text-white">{counts.PENDING}</span>
            )}
            {t.charAt(0) + t.slice(1).toLowerCase()}
            {t !== 'PENDING' && counts[t] > 0 && <span className="text-slate-400">({counts[t]})</span>}
          </button>
        ))}
      </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-slate-400">
                    {a.kycDocType && <span>KYC: {a.kycDocType}</span>}
                    {a.payoutType && <span>Payout: {PAYOUT_LABEL[a.payoutType]}</span>}
                    {a.creatorAppliedAt && <span>Applied {timeSince(a.creatorAppliedAt)}</span>}
                  </div>
                </div>
                <span className="shrink-0 text-sm text-slate-300 transition group-hover:text-brand-500">→</span>
              </div>
            </button>
          ))}
        </div>
